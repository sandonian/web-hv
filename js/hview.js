...
$(function () {
    ...
    let currentJSZip; /* saved for resetting view hierarchy */
    ...
    var loadImage = function (node) {
        ...
        viewController.captureView(node.name).then(imageData => {
            ...
            if (e == NO_IMAGE_LOADED_MESSAGE && $("#border-box").css('background-image') == 'none'  && $("#image-preview").css('background-image') == 'none') {
    ...
    var renderList = function (root, vListContent = $("#vlist_content").empty(), boxContent = $("#border-box").empty()) {
    ...
    multiHViewAction = function (jsZip) {
        showViewHierarchyUX()
        if ($("#tl-range-container").hasClass("hidden")) {
            $(".slider-group").toggleClass("visible hidden")
            $("#vlist, #border-box").addClass("multi-page")
        }

        currentJSZip = jsZip
        /* Setting viewController to null if this multiHViewAction call is a reset button click */
        viewController /* OfflineServiceController | null */ = null
        ...
        for(const [filename, file] of Object.entries(jsZip.files)) {
            /* JSZip.files references subFolders and hidden folders in addition to the desired child folders.
            ...
            if (!(filename.startsWith("_")) && file.dir && !(filename.endsWith("img/"))) {
                ...
                currentAppInfo /* Custom Object */ = { type: TYPE_ZIP, data: subZip, config: subConfig, name: subConfig.title }

                if (viewController == null) {
                    viewController = createViewController(currentAppInfo)

                    /* Some UI setup needs to run after the appInfo / view controller has been initialized. */
                    setupCustomCommandButton(viewController)
                ...
                viewController.loadViewList().then(rootNode => {
                    ...
                    /* if the first view list has been loaded properly, then show it on the screen */
                    if (vListJQueries.length == 1) {
        ...
        function migrateSelectedState(index /* Integer */) {
            if (nodeMap == null) {
                ...
                function initializeNodeMap(nodes /* ViewNode[] */, rootNodeIndex /* Integer | null */ = null) {
                    ...
                    for (let i = 0; i < nodes.length; i++) {
                        ...
                        initializeNodeMap(nodes[i].children, rootNodeIndex)
                    }
                }
                initializeNodeMap(rootNodes)
            }

            function helper(clazz /* String */, containerId /* String */, elementAccessor /* String */) {
            ...
            helper(CLS_SELECTED, "vlist", "el")
            helper(CLS_LAST_SELECTED, "vlist", "el")
            helper(CLS_SELECTED, "border-box", "box")
            helper(CLS_LAST_SELECTED, "border-box", "box")
        }
        ...
        function switchViewHierarchy(newIndex /* Integer */, oldIndex /* Integer */) {
            function helper(htmlContainer /* JQuery selector String */, rootJQueries /* JQuery[] */) {
                rootJQueries[oldIndex] = $("<div>").append($(htmlContainer).children().detach())
                rootJQueries[newIndex].children().appendTo(htmlContainer)
            }
            helper("#vlist_content", vListJQueries)
            helper("#border-box", boxJQueries)
        }
        ...
        showContext(menu, function (el) {
            switch(this.id) {
                ...
                case 2:
                    ...
                    if ($("#tl-range-container").hasClass("visible")) {
                        multiHViewAction(currentJSZip)
                    } else {
